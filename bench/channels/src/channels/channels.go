...
/** Server goroutine ("net latency" neglected).
 * @param id Server ID
**/
func server(id uint) {
    for {
...
func main() {
    ...
    var only_results bool    // Only print results
    ...
    { // Command line parsing
        ...
        flag.BoolVar(&only_results, "o", false, "Only print results, separated by '\\n'")
        flag.Parse()
    ...
    { // Initialize
        if !only_results {
            ...
            fmt.Println("- payload size =", unsafe.Sizeof(payload(0)), "bytes")
            fmt.Println("- message size =", unsafe.Sizeof(message{0, make(chan(message))}), "bytes")
        }
    ...
    { // Statistics (duration is here in ns)
        if only_results {
            ...
            fmt.Printf("- average latency ~%.3f µs/msg\n", avgl) // Average latency for goroutine A -> goroutine B
        }
    }
}
